...
export class SwapSlotsComponent extends FeatureComponent<ResourceId> implements OnDestroy {
    ...
    @Output('parameters') parameters$: Subject<SwapSlotParameters>;
    @Output('configApplied') configApplied$: Subject<SrcDestPair>;
    ...
    constructor(
        ...
        this.featureName = 'deploymentslots/actions/swap';
    ...
    private _processLoadingResults(
        siteResult: HttpResult<ArmObj<Site>>,
        slotsResult: HttpResult<ArmArrayResult<Site>>) {
            ...
            this._cacheService.postArm(params.uri, null, null, params.content)
                ...
                .subscribe(r => {
                    ...
                    if (!r.success) {
                        ...
                        this.progressMessage = this._translateService.instant(PortalResources.swapFailure, { operation: operation, error: JSON.stringify(r.error) });
